...
Mismatch는 UART RX가 8비트 데이터를 잘못 복원한 결과가 아니라, FIFO Full 상태에서 일부 바이트가 유실된 이후 Scoreboard의 기대 데이터와 실제 출력 데이터의 비교 순서가 어긋나 연속적으로 발생한 결과임. Pending은 입력되었지만 Loopback 출력으로 확인되지 않은 기대 데이터임.

각 frame은 Start bit에서 수신 시점을 다시 판별하므로 baud timing 오차가 frame 간 계속 누적되는 구조는 아님. 따라서 해당 결과는 RX sampling margin만을 나타내는 것이 아니라, baud rate가 다른 연속 Loopback 환경에서의 처리율과 FIFO 수용 범위를 함께 검증한 결과임.
